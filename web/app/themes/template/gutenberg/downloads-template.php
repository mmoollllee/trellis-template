...
<?php while (have_rows('downloads')):
	...
	<section class="download row">
		...
		<?php foreach ($files as $file): ?>
			<li data-icon-left="d"><?php echo $file['description']
       ? $file['description'] . ' '
       : ''; ?><a target="_blank" href="<?php echo $file[
    'url'
]; ?>" title="<?php echo $file[
    'filename'
]; ?> herunterladen/öffnen"><?php echo $file[
     'title'
 ]; ?> <small>.<?php echo $file['subtype']; ?></small></a></li>
		<?php
...
<?php
	...
	<section class="download info row">
      ...
      $link_target = $link['link']['target']
          ...
          : '_self';
      ?>
			<li data-icon-left="d"><?php echo '<a target="_blank" href="' .
       esc_url($link_url) .
       '" target="' .
